...
namespace shader_manage
{
    ...
    public static class shader_col
    {
        ...
        public static void bar_controller(RoomCamera s)
        {
            ...
            if (shader_manage.shader_col.f_sprite.alpha <= 0.75f)
            {

                s.effect_desaturation = 0.25f;

            }
            if (shader_manage.shader_col.f_sprite.alpha <= 0.50f)
            {

                s.effect_desaturation = 0.50f;
                shader_manage.shader_col.f_sprite.color = Color.yellow;
                s.effect_darkness = 0.10f;

            }
            if (shader_manage.shader_col.f_sprite.alpha <= 0.25f)
            {

                s.effect_desaturation = 0.75f;
                shader_manage.shader_col.f_sprite.color = Color.red;
                s.effect_darkness = 0.15f;

            }
            if (shader_manage.shader_col.f_sprite.alpha <= 0.10f)
            {
        ...
        public static void bar_effect_desaturation(RoomCamera s, Player self)
        {
            ...
            if (self.slugcatStats.name == marshaw)                                      //if the slugcat is the MARSHAW
            {
                ...
                shader_manage.shader_col.f_sprite.isVisible = true;                               // makes visible only for MARSHAWWWWWWWWWWW
            ...
            else                                                                        // else if its not Marshaw
            {
                ...
                s.ReturnFContainer("HUD").RemoveChild(shader_manage.shader_col.f_sprite);         // remove the bar :)
